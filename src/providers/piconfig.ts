 ...
 */
export function readPiProviders(path = piModelsJsonPath()): PiConfigProvider[] {
  if (!existsSync(path)) return [];
  let raw: RawPiConfig;
  try {
    raw = JSON.parse(readFileSync(path, "utf8")) as RawPiConfig;
  } catch {
    return [];
  }
  ...
  return Object.entries(raw.providers ?? {}).map(([name, entry]) => {
    const models: PiConfigModel[] = (entry.models ?? [])
      .map((m) => ({
...
export function findPiProvider(name: string, path = piModelsJsonPath()): PiConfigProvider | null {
 ...
 */
export class PiRegisteredProvider implements Provider {
  readonly name: string;
  readonly displayName: string;
  readonly baseUrl: string;
  private readonly config: PiConfigProvider;
  ...
  async health(): Promise<ProviderHealth> {
    ...
    const models = await fetchJson<{ data?: unknown[] }>(`${this.baseUrl}/models`, {
      timeoutMs: 3000,
    });
    if (!models) {
  ...
  async ensureContext(modelId: string, contextLength: number): Promise<string> {
    ...
    if ((info.effective ?? 0) < contextLength) {
      throw new Error(
        `${modelId} is registered with pi for ${info.effective ?? "an unknown"} tokens, below the required ${contextLength}. ` +
