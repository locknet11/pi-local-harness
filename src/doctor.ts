...
export interface DoctorReport {
  sections: Array<{ title: string; checks: Check[] }>;
  ok: number;
  warn: number;
  fail: number;
}

export const piModelsJsonPath = (): string =>
...
export async function diagnose(
  ...
  project.push(
    existsSync(join(cwd, config.agentsFile))
  ...
  if (resolved) {
    const label =
      resolved.source === "declared" ? config.testCmdFile : resolved.source;
    project.push(
      commandExecutable(resolved.command)
...
export function renderReport(report: DoctorReport): string {
  ...
  const icon = (l: CheckLevel) =>
    l === "ok" ? color.green("✔") : l === "warn" ? color.yellow("!") : color.red("✖");
  ...
  lines.push("");
  lines.push(
    color.dim(`${report.ok} ok, ${report.warn} warnings, ${report.fail} problems`),
  );
  lines.push("");
  return lines.join("\n");
}
