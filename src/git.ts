...
async function git(ctx: GitContext, args: string[], timeoutSeconds = 120) {
  return run("git", args, { cwd: ctx.cwd, timeoutSeconds });
}

export async function isRepo(ctx: GitContext): Promise<boolean> {
  const r = await git(ctx, ["rev-parse", "--is-inside-work-tree"], 15);
  return r.code === 0 && r.stdout.trim() === "true";
}

export async function head(ctx: GitContext): Promise<string> {
  const r = await git(ctx, ["rev-parse", "--short", "HEAD"], 15);
  return r.code === 0 ? r.stdout.trim() : "";
}

export async function isDirty(ctx: GitContext): Promise<boolean> {
  const r = await git(ctx, ["status", "--porcelain"], 30);
  return r.stdout.trim() !== "";
}

export async function ensureIdentity(ctx: GitContext): Promise<void> {
...
export async function initRepo(ctx: GitContext, writeGitignore: () => void): Promise<void> {
  if (await isRepo(ctx)) {
...
export async function checkpoint(ctx: GitContext, label: string): Promise<string> {
  if (!(await isRepo(ctx))) return "";
...
export async function commitFeature(
  ...
  if (!(await isRepo(ctx))) return null;
  await git(ctx, ["add", "-A"], 60);
...
export async function rollbackTo(ctx: GitContext, ref: string): Promise<boolean> {
  if (!ref || !(await isRepo(ctx))) return false;
...
export async function ensureBranch(ctx: GitContext, branch: string): Promise<void> {
  if (!branch || !(await isRepo(ctx))) return;
...
export async function diffStat(ctx: GitContext, ref: string): Promise<string> {
  if (!ref) return "";
...
export const TEST_FILE_PATTERN =
  /(^|\/)(tests?|spec|__tests__)\/|(^|\/)test_[^/]*$|_test\.[a-z]+$|\.(test|spec)\.[jt]sx?$|Tests?\.(java|kt|cs)$|_spec\.rb$/;
 ...
 */
export async function testFilesChangedSince(ctx: GitContext, ref: string): Promise<number> {
  if (!ref || !(await isRepo(ctx))) return 0;
