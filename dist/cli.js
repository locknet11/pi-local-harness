...
async function resolveProvider(options, config) {
    const name = options.provider ?? config.provider;
    if (name)
        return createProvider(name);
    const detected = await detectProvider();
    if (!detected) {
        throw new Error("No local backend is running. Start LM Studio (`lms server start`) or Ollama (`ollama serve`), or pass --provider.");
    }
...
async function main() {
    ...
    switch (command) {
        ...
        case "spec": {
            ...
            for (const f of features) {
                process.stdout.write(`  ${statusMark(f.status)}  #${String(f.index).padEnd(3)} ${String(f.status).padEnd(11)} ${f.name}\n`);
            }
    ...
    switch (command) {
        case "models": {
            ...
            if (!health.running) {
                ...
                if (!(await provider.start())) {
                    ...
                    for (const a of provider.advice())
                        log.detail(a);
                    return 1;
                }
            }
            ...
            for (const m of models) {
                ...
                process.stdout.write(`  ${live ? color.green("●") : color.dim("○")} ${m.id.padEnd(38)} ${color.dim(bits.join("  "))}` +
        ...
        case "setup-model": {
            ...
            if (backupPath)
                log.detail(`backup: ${backupPath}`);
            log.ok(`Written to ${piModelsJsonPath()}`);
            log.ok((await piSeesModel(config.piBin, provider.name, modelId))
        ...
        case "tune-ctx": {
            ...
            log.ok(`Model to use: ${finalId}${info.effective ? ` (context ${info.effective})` : ""}`);
            if (finalId !== modelId) {
        ...
        case "probe": {
            ...
            log.detail("It most likely returns the tool-call JSON as text. Declaring 'tools' is not enough — that comes from the chat template, not the model.");
            if (result.finalText)
                log.detail(`model said: ${result.finalText.slice(0, 300)}`);
            return 1;
        }
        case "doctor": {
            ...
            for (const a of provider.advice())
                process.stdout.write(`  ${color.dim("· " + a)}\n`);
            process.stdout.write("\n");
    ...
    installSignalHandlers((count) => {
        if (count === 1)
            log.warn("Stop requested. Finishing the current step… (again = force kill)");
    });
    try {
        const ctx = { cwd, config, tempDir };
        ...
        if (command === "init" || command === "build") {
            ...
            await git.initRepo({ cwd }, () => {
                ...
                if (!existsSync(gitignorePath))
                    writeFileSync(gitignorePath, git.defaultGitignore);
            });
            ...
            if (options.brief) {
                ...
                const brief = await interview(cwd.split("/").pop() ?? "project", config.featureTarget);
                config.featureTarget = brief.featureTarget;
                writeBrief(ctx, brief);
                log.ok(`Brief written to ${config.briefFile}`);
            }
            if (!(await generateAgentsFile(ctx)))
                return 1;
            if (!(await generateSpecFile(ctx)))
                return 1;
            const docsCommit = await git.commitFeature({ cwd }, "F000", `project documents (${config.agentsFile}, ${config.specFile})`, "COMPLETED");
            if (docsCommit)
                log.ok(`commit — ${docsCommit}`);
        ...
        await git.ensureBranch({ cwd }, config.gitBranch);
        resetStale(specPath, (f) => log.warn(`Feature #${f.index} was left IN_PROGRESS by an earlier run → PENDING`));
        const features = readSpec(specPath);
        log.step(`Backlog: ${features.length} features · ${provider.name}/${config.model}`);
        ...
        for (const f of finalFeatures) {
            log.plain(`  ${statusMark(f.status)}  #${String(f.index).padEnd(3)} ${String(f.status).padEnd(11)} ${f.name}`);
        }
        const totals = summarize(finalFeatures);
        log.blank();
        log.plain(`  total=${totals.total}  completed=${totals.completed}  unverified=${totals.unverified}  failed=${totals.failed}  pending=${totals.pending}`);
        log.blank();
