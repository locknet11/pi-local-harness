...
export function declaredTestCommand(ctx) {
...
const has = (cwd, p) => existsSync(join(cwd, p));
export function detectTestCommand(cwd) {
    ...
    if (has(cwd, "Makefile")) {
        try {
            if (/^test:/m.test(readFileSync(join(cwd, "Makefile"), "utf8")))
                return "make test";
        }
        catch {
            /* unreadable Makefile */
...
export function resolveTestCommand(ctx, opts) {
    if (opts.configured)
        return { command: opts.configured, source: "config" };
    if (opts.featureTest)
        return { command: opts.featureTest, source: "feature" };
    const declared = declaredTestCommand(ctx);
    if (declared)
        return { command: declared, source: "declared" };
    const detected = detectTestCommand(ctx.cwd);
    return detected ? { command: detected, source: "detected" } : null;
}
export async function runVerification(ctx, command, timeoutSeconds, outFile) {
 ...
 */
export function excerpt(output, maxLines = 60) {
