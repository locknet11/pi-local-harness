 ...
 */
export function readCloudProviders(path = piModelsStorePath()) {
    if (!existsSync(path))
        return [];
    let raw;
    try {
...
export function findCloudProvider(name, path = piModelsStorePath()) {
 ...
 */
export function credentialStatus(name, authPath = piAuthJsonPath()) {
    const envVar = ENV_KEY[name];
    if (existsSync(authPath)) {
        try {
            const auth = JSON.parse(readFileSync(authPath, "utf8"));
            if (auth[name])
                return { present: true, source: "auth.json", ...(envVar ? { envVar } : {}) };
        }
        catch {
...
export class CloudProvider {
    ...
    async contextInfo(modelId) {
        ...
        if (!model || model.contextWindow === undefined)
            return {};
        return {
    ...
    async ensureContext(modelId, contextLength) {
        ...
        if (info.effective !== undefined && info.effective < contextLength) {
            throw new Error(`${modelId} is catalogued with a ${info.effective}-token context, below the required ${contextLength}. ` +
