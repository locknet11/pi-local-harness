...
import { NO_TESTS_NUDGE, NO_WRITES_NUDGE, failedVerificationSection, featurePrompt, } from "./prompts.js";
import { sleep, stopFlag } from "./proc.js";
import { readSpec, setStatus } from "./spec.js";
import { formatDuration, log } from "./ui.js";
import { excerpt, resolveTestCommand, runVerification } from "./verify.js";
export async function processFeature(ctx, feature, checkpoint) {
    const { config } = ctx;
    const gitCtx = { cwd: ctx.cwd };
    const verifyCtx = { cwd: ctx.cwd, testCmdFile: config.testCmdFile };
    ...
    for (let attempt = 1; attempt <= config.maxRetries; attempt += 1) {
        ...
        if (lastNoWrites)
            prompt += NO_WRITES_NUDGE;
        if (lastNoTests)
            prompt += NO_TESTS_NUDGE;
        ...
        const verification = await runVerification(verifyCtx, testCommand, config.testTimeout, join(ctx.tempDir, `test.f${feature.index}.log`));
...
export async function runLoop(ctx, optionsInput = {}) {
    ...
    for (;;) {
        ...
        if (pendingOnlyId) {
            ...
            if (!feature)
                throw new Error(`No feature with id '${pendingOnlyId}' in ${config.specFile}`);
        ...
        if (config.unloadBetweenFeatures && ctx.provider) {
            if (await ctx.provider.unload(config.model))
                log.detail("model unloaded to free memory");
        }
        await sleep(config.cooldown);
    }
}
/** Ensure the spec file exists and is parseable before starting a run. */
export function requireSpec(cwd, specFile) {
