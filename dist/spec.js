...
export function validateSpec(text) {
    ...
    for (const f of features) {
        ...
        if (!f.name)
            errors.push(`Feature #${f.index} has no name.`);
        if (!f.status)
            errors.push(`${label}: missing "status:".`);
        else if (!FEATURE_STATUSES.includes(f.status)) {
            errors.push(`${label}: invalid status "${f.status}" (expected one of ${FEATURE_STATUSES.join(", ")}).`);
        }
        if (!f.id)
            ...
            seenIds.add(f.id);
        if (f.acceptance.length === 0)
            errors.push(`${label}: missing the "acceptance:" list.`);
    }
    ...
    for (const f of features) {
        for (const dep of f.depends) {
            ...
            if (depIndex === undefined) {
                ...
                errors.push(`Feature #${f.index} (${f.name}): depends on "${dep}", which is defined later. Order features so dependencies come first.`);
            }
        }
    }
    return errors;
}
const isDone = (s) => s === "COMPLETED" || s === "UNVERIFIED";
/** First PENDING feature whose dependencies are all satisfied. */
export function nextPending(features) {
...
export function hasBlockedPending(features) {
 ...
 */
export function setStatus(file, index, status) {
    const text = readFileSync(file, "utf8");
    const before = parseSpec(text);
    const target = before.find((f) => f.index === index);
    if (!target)
        throw new Error(`Feature #${index} not found in ${file}`);
    if (target.statusLineNo < 0)
        throw new Error(`Feature #${index} has no "status:" line`);
    const lines = text.split("\n");
...
export function resetFrom(file, from) {
    let count = 0;
    for (;;) {
...
export function resetStale(file, onReset) {
    let count = 0;
    for (;;) {
        const features = readSpec(file);
